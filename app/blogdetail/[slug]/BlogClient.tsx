...
export default function BlogDetailClient({ blog }: { blog: any }) {
  ...
  const formatContent = (content: string) => {
    ...
    return lines.map((line, index) => {
      ...
      if (
        trimmedLine.includes("depreciates") ||
        trimmedLine.includes("CapEx") ||
        trimmedLine.includes("OpEx")
      ) {
      ...
      if (
        trimmedLine.includes("Call / WhatsApp") ||
      ...
      if (trimmedLine.endsWith("?")) {
        return (
          <div
            key={index}
            className="my-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border-l-4 border-blue-500"
          >
